...
CREATE TABLE played (
  ...
  PRIMARY KEY (played, track_id, album_id, artist_id),
...
SELECT *
FROM track
WHERE track_name LIKE 'F_n_%';
SELECT album_name
FROM album
WHERE album_name > 'c' AND album_name < 'm';
SELECT album_name
FROM album
WHERE album_name LIKE 'l%' OR album_name LIKE 's%g';

SELECT *
FROM album
WHERE album_name LIKE 'l%' OR album_name LIKE 's%' OR album_name LIKE 'p%';
SELECT *
FROM album
...
SELECT *
FROM album
WHERE album_id > 2 AND album_id != 4 AND album_id != 6;
SELECT *
FROM album
WHERE album_name NOT LIKE 'l%';
SELECT *
FROM album
WHERE album_name LIKE 's%' AND album_name NOT LIKE '%)';
SELECT *
FROM artist
WHERE artist_name LIKE '%es' XOR artist_name LIKE 'The%';
SELECT *
FROM album
WHERE album_name LIKE 'brotherhood';
SELECT *
FROM track
WHERE album_id = 7;
...
FROM track
WHERE time > 10
      OR track_name LIKE 'The Perfect Kiss [%'
...
SELECT
  ...
  track_name
FROM album
  INNER JOIN track USING (album_id, artist_id);
SELECT
  played,
  track_name
FROM track
...
SELECT sum(time)
FROM track
WHERE artist_id = 1 AND album_id = 7;
SELECT sum(time)
FROM artist
  INNER JOIN track USING (artist_id)
WHERE artist_name LIKE "%Rolling%";

UPDATE album
SET album_name = "Substance 1987 (Disc 2)"
...
insert into album (artist_id, album_id, album_name) VALUES
  (7, 1, 'Leftism');
insert into track (artist_id, album_id, track_id, track_name, time) VALUES
  (7, 1, 1, 'Release the Pressure', '7.39'),
  ...
  (7, 1, 10, 'Bonus Track', '1.22');


select sum(time) from track where artist_id = 7 and album_id = 1;
select sum(time) from track inner join artist where artist_name like '%Rolling Stones%';
update track inner join album set time = '6.22'
where album_name like 'Leftism' and track_name like 'Original';
delete from track where track_name like 'Bonus Track' and artist_id = 7;
select track_name, artist_name from track inner join artist using (artist_id)
